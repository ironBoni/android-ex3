    ...
    public class ChatHub : Hub {
        ...
        public async Task SendMsg(string from, string content, string to)
        {
            ...
            if (userToConnection.ContainsKey(to))
            {
                ...
                foreach (var conId in connectionIds)
                    await Clients.Client(conId).SendAsync(ReceiveMessage, new MessageResponse(id, content, DateTime.Now, true, from, chat.Id, to));
            }
            else
                await Clients.AllExcept(Context.ConnectionId)
                .SendAsync(ReceiveMessage, new MessageResponse(id, content, DateTime.Now, true, from, chat.Id, to));
        }
